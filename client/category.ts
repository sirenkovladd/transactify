...
async function updateTransactionsCategory(transactionIds: number[], category: string) {
    const response = await fetch('/api/transactions/category', {
        ...
        body: JSON.stringify({ transaction_ids: transactionIds, category: category }),
    });
    if (!response.ok) {
...
export function setupCategoryModal() {
    ...
    const categorySelect = document.getElementById('category-modal-select') as HTMLSelectElement;
    ...
    openCategoryModal = (transactionIDs: number[], title: string = 'Change Category') => {
...
export let openCategoryModal: (transactionIDs: number[], title?: string) => void;
