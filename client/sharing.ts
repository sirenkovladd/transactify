...
import { addConnection, generateToken, getSubscriptions, getTokens, revokeToken, type Subscription, unsubscribe } from "./common";
...
export function renderSharingSettings(container: HTMLElement) {
  ...
  const subscriptions = van.state<{ subscribers: string[], subscriptions: Subscription[] }>({ subscribers: [], subscriptions: [] });
  ...
  const Tokens = () => div(
    ...
    button({ onclick: async () => { await generateToken(); fetchTokens(); } }, "Generate New Token")
  );

  const Subscriptions = () => div(
  ...
  const AddConnection = () => {
    ...
    return div(
      h3("Add Connection"),
      tokenInput,
      button({ onclick: async () => { await addConnection(tokenInput.value); fetchSubscriptions(); } }, "Add")
    )
  }
